...
\State $H_0\gets\HSHA{Z[0]}$\Comment{Hash the key ring identifier.}
\State $H_1\gets\HSHA{\texttt{userid+domain}}$\Comment{Hash the userid and the domain name.}
...
\State $K_y\gets\ERSA{K_s}{K_{le}}$	\Comment{Encrypt $K_s$ with login server public key.}
\State $K_d\gets\DAES{K_h}{K_{LE}}$ \Comment{Decrypt to get dummy key.}
\State $K_u\gets\EAESm{K_s}{S[0]}$ \Comment{This is the new user key.}
\State $K_x\gets\xor{K_u}{K_d}$ \Comment{Encrypt this with the decrypted dummy key.}
...
\State $A_u\gets\EAES{R_u+C_1}{K_u}$\Comment{Compute a challenge.}
\State $B_u\gets\xor{K_u}{\HSHAm{R_u}}$\Comment{And the response also.}
\State $Q_u,j\gets 0,0$\Comment{Initialize.}
\State $B_x,P_x\gets$\Call{SendToWebServer}{$U_h,A_u$}\Comment{Wait for $B_s$ and $P_s$.}
\State $B_s\gets\DAES{B_x}{R_u}$
\While{$B_s\ne 0$}\Comment{Website is trying keys for us.}
\If{$\VRSA{B_u+\xor{B_s}{K_u}}{K_{ls}}$}	\Comment{Verify $B_s$. Right key has been found!}
\State $P_s\gets\DAES{P_x}{R_u}$
\State $R_s\gets\xor{P_s}{K_u}$
\State $Q_u\gets\xor{R_u}{\HSHAm{R_s}}$\Comment{Compute response to $P_s$.}
\Else
...
\State $B_x,P_x,Q_s\gets$\Call{HSM}{$A_u,K_y,n,v,i$} \Comment{Call this function on login server.}
\If{$Q_s\ne 0$}
\State $Q_u\gets$\Call{SendToUser}{$B_x,P_x$}\Comment{Wait for $Q_u$.}
...
\State $K_s\gets\mod{\DRSA{K_y}{K_{LE}}}{2^n}$	\Comment{Decrypt the site key $K_s$.}
\State $K_u\gets\EAESm{K_s}{S[i]}$	\Comment{Temporarily regenerate user key.}
\State $R_u,C_1\gets\DAES{A_u}{K_u}$	\Comment{Decrypt user random and certificate.}
\If{$C_1 = C_2$}
...
\State $B_s\gets\SRSA{B_s}{K_{LS}}$	\Comment{Replace hash with signature.}
\EndIf
\If{$i>0$}\Comment{Not the first attempt with $K_s$.}
\State $R_s\gets\EAESm{K_s}{S[0]}$\Comment{Send user a new key.}
...
\State $Q_s\gets\xor{R_u}{\HSHAm{R_s}}$\Comment{And the response also.}
\Else\Comment{Certificates do not match.}
\State $B_s,P_s,Q_s\gets0,0,0$\Comment{It's game over.}
\EndIf
\Else\Comment{Array $S$ is exhausted.}
\State $B_s,P_s,Q_s\gets0,0,0$\Comment{It's game over.}
\EndIf
\State $B_x\gets\EAES{B_s}{R_u}$	\Comment{Encrypt with $R_u$.}
\State $P_x\gets\EAES{P_s}{R_u}$	\Comment{Encrypt with $R_u$.}
\State \Return $B_x, P_x, Q_s$\Comment{Return these to the web server.}
\EndProcedure
\end{algorithmic}
\end{algorithm}
